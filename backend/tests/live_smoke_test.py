...
BASE_URL = os.getenv("MODLAB_BASE_URL", "http://127.0.0.1:8000").rstrip("/")
...
def build_csv_bytes() -> bytes:
    ...
    for index in range(96):
        writer.writerow([index / 1200, 0.75 * ((-1) ** index)])
...
def main() -> None:
    with httpx.Client(base_url=BASE_URL, timeout=120.0) as client:
        ...
        models_before = client.get("/models")
        assert models_before.status_code == 200, models_before.text
        ...
        models_after = client.get("/models")
        assert models_after.status_code == 200, models_after.text
    ...
    print(f"Live smoke test passed against {BASE_URL}: / /health /config /device /self-check /workflow/overview /report/summary /models /models/active /train /predict/generated /predict /upload/iq /predict/file + invalid upload handling")
